...
export default function JoinGroupPage() {
  ...
  // Check geo-fence when code is available
  useEffect(() => {
    const trimmedCode = code.trim().toUpperCase();
    if (trimmedCode.length !== 6) return;
    groupOrderService.getByCode(trimmedCode).then((g) => {
      setGeoFenceEnabled(!!g.restaurant?.geoFenceEnabled);
    }).catch(() => {});
  }, [code]);
  ...
  const handleJoin = useCallback(async () => {
    ...
    setError('');
    try {
      // First validate the phone format via backend — look up group to get restaurantId/tableId
      const groupInfo = await groupOrderService.getByCode(trimmedCode);
      if (groupInfo.restaurantId && groupInfo.tableId) {
        await otpService.sendOtp(fullPhone, groupInfo.restaurantId, groupInfo.tableId);
      }

      const participant = await groupOrderService.join(trimmedCode, {
        name: trimmedName,
        phone: fullPhone,
      }, getCoords());
      // Store participant info for this group session
      sessionStorage.setItem(`group:${trimmedCode}:participantId`, participant.id);
      sessionStorage.setItem(`group:${trimmedCode}:participantName`, participant.name);
      sessionStorage.setItem(`group:${trimmedCode}:isHost`, 'false');
      navigate(`/group/${trimmedCode}`);
    } catch (err: any) {
      setError(err.message || t('group.failedJoin'));
    } finally {
      setLoading(false);
    }
  }, [code, name, countryCode, phoneNumber, navigate]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-amber-50 flex items-center justify-center px-4">
      ...
      >
        ...
        <div className="text-center mb-6">
          ...
          <h1 className="text-2xl font-bold text-gray-900">{t('group.joinGroup')}</h1>
          <p className="text-sm text-gray-500 mt-1">{t('group.joinHint')}</p>
        </div>

        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('group.groupCode')}</label>
            <input
              type="text"
              maxLength={6}
              value={code}
              ...
              className="w-full px-4 py-3 border border-gray-200 rounded-xl text-center text-2xl font-mono tracking-[0.3em] uppercase focus:ring-2 focus:ring-orange-300 focus:border-orange-400 outline-none transition-all"
            />
          </div>
          ...
          <button
            onClick={handleJoin}
            disabled={loading || geoBlocked || !code.trim() || !name.trim() || phoneNumber.replace(/\D/g, '').length < 5}
